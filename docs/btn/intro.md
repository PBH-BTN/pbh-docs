...
BTN 服务端提供了多种能力，PeerBanHelper 客户端可以与这些服务进行交互，以获得更好的防护效果：

### BTN IP 查询服务
BTN IP 查询服务允许 PeerBanHelper 查询 BTN 服务器以获取有关特定 IP 地址的更多信息。PeerBanHelper 功能模块可以使用此服务提供的能力获取有关特定 IP 地址在 BTN 上的活动概要。

### 重新配置能力
“重新配置” 能力允许 BTN 服务器通知 PeerBanHelper 在间隔一定时间后与 BTN 服务器联系，刷新使用的配置文件，以获取远程服务器的配置更改。

### 心跳更新
该能力允许 BTN 网络请求 PeerBanHelper 每隔一定时间向 BTN 服务器发送一个心跳请求，以更新在 BTN 网络上的状态。有时该功能还被用来检测您的可用 IP 地址，以便更新在 BTN 网络上的记录。这取决于服务器的要求。

### 提交封禁列表
“提交封禁列表” 能力允许 BTN 服务器定期接收 PeerBanHelper 的封禁列表和封禁时的数据快照。这些数据有助于 BTN 服务器分析网络上的恶意行为，并动态生成反吸血规则阻断这些恶意行为。

### 云端规则
“云端规则” 能力允许 BTN 服务器向 PeerBanHelper 定期提供由远程服务器生成的反吸血规则，作为本地反吸血规则的补充。

### 提交种群跟踪数据
“提交种群跟踪数据” 是 “提交历史记录” 和 “提交快照数据” 的继任者。新的 “提交种群跟踪数据” 以一种更加服务器友好的格式追踪、存储和提交 Peers 数据，以减轻 BTN 服务器的数据聚合分析压力。该能力允许 BTN 服务器定期接收 PeerBanHelper 所跟踪的下载器上的 Peers 活动和最后一次记录的活动数据快照。这些数据有助于 BTN 服务器分析网络上的恶意行为，并动态生成反吸血规则阻断这些恶意行为。同时，这些数据还有助于分析是否存在利用 PeerBanHelper 反吸血功能的漏洞，绕过反吸血机制的在野利用。

### IP 拒绝列表
IP 拒绝列表能力允许远程服务器下发需要拦截的 IP/CIDR 地址列表。位于此列表内的连接将立刻被 PeerBanHelper 封禁。
